...
$site_top = "http://yuumin.chu.jp/itsy/index.html";
...
$from = "jane.doe3wtp4649@gmail.com";
...
$thanksPage = "http://yuumin.chu.jp/itsy/index.html";
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $Email && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
				$errm .= "<p class=\"error_messe\">【".$key."】 is wrong format</p>\n";
				$empty_flag = 1;
			}
		}
	}
...
<?php if($empty_flag == 1){ ?>
<div align="center">
<h4>There is an error in the input. After confirming the following, please fix it by "back" button.</h4>
<?php echo $errm; ?><br /><br /><input type="button" value=" back " onClick="history.back()">
</div>
<?php }else{ ?>
<h3>確認画面</h3>
<p align="center">Please press the "Send" button if there is a mistake in the following contents.</p>
...
<?php }else{ ?>
thank you<br />
The submission completed successfully.<br /><br />
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div align="center"><h4>There is an error in the input. After confirming the following, please fix it by "back" button.</h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input type="button" value=" back " onClick="history.back()"></div>
<?php 
...
function requireCheck($require){
	...
	foreach($require as $requireVal){
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<p class=\"error_messe\">【".h($key)."】is required Fields.</p>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
		...
		if($existsFalg != 1){
				$res['errm'] .= "<p class=\"error_messe\">【".$requireVal."】is required Fields.</p>\n";
				$res['empty_flag'] = 1;
		}
	}
...
function refererCheck($Referer_check,$Referer_check_domain){
	if($Referer_check == 1 && !empty($Referer_check_domain)){
		if(strpos($_SERVER['HTTP_REFERER'],$Referer_check_domain) === false){
			return exit('<p align="center">The domain of this file does not match the domain of the form page. </p>');
		}
	}
}
function copyright(){
